// Dev Multi Gift Card Data Form (dev-multi-gift-card-data-form)
(RaiselyComponents, React) => {
  ...
  const LIST_ERROR_INVALID_EMAIL = "invalid-email-in-list";
  ...
  const emailIsValid = (email) => {
    return email.search(/.+@.+\..+/) > -1 && email.search(/^\S+@\S+\.\S+$/) > -1;
  }
    ...
    function csvToArray(str) {
      ...
      if (headerIndex > -1) {
        ...
        console.log("~~ values:", values);
          const dataObj = headers.reduce((object, header, index) => {
        console.log("~~ object:", object, "header:", header, "index:", index);
            object[header] = values[index];
            return object;
          }, {});
            const email = dataObj[recipientCSVEmail];
            const validEmail = emailIsValid(email);
            console.log("~~ validEmail:", validEmail);
            if(email.indexOf('"') !== -1 || !validEmail) {
              setListError(LIST_ERROR_INVALID_EMAIL);
            }
            return dataObj;
        });
      } else {
  ...
  return (props) => {
    ...
    let listError = "";
    switch(listErrorVal) {
      case LIST_ERROR_INVALID_EMAIL:
        listError = values.errorListEmailInvalidFormat;
        break;
